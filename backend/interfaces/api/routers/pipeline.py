...
async def get_pipeline_readiness(
    ...
    asm_total: int = (
        await db.scalar(
            select(func.count(AssessmentModel.id)).where(AssessmentModel.organization_id == org_id)
        )
        or 0
    )
    asm_draft: int = (
    ...
    if sup_total == 0:
        s1_missing.append(
            MissingItem(
                type="action", label="Noch keine Lieferanten angelegt", count=0, href="/suppliers"
            )
        )
    else:
        if sup_no_nace > 0:
        ...
        if sup_no_country > 0:
            s1_missing.append(
                MissingItem(
                    type="data",
                    label="Herkunftsland fehlt",
                    count=sup_no_country,
                    href="/suppliers",
                )
            )
    steps.append(
    ...
    if asm_total == 0:
        s2_missing.append(
            MissingItem(
                type="action",
                label="Kein Assessment geplant — Zeitplan erstellen",
                count=0,
                href="/assessments/schedules",
            )
        )
    elif asm_draft > 0:
        s2_missing.append(
            MissingItem(
                type="action",
                label="Assessments noch im Entwurfsstatus",
                count=asm_draft,
                href="/assessments",
            )
        )
    ...
    steps.append(
        StepReadiness(
            ...
            score=100
            if not s3_missing
            else max(0, 100 - int((asm_no_evidence / max(asm_total, 1)) * 100)),
            open_count=asm_total,
            missing=s3_missing,
        )
    )
    ...
    if asm_total > 0 and fin_total == 0:
        s4_missing.append(
            MissingItem(
                type="action", label="Keine Findings dokumentiert", count=0, href="/findings"
            )
        )
    steps.append(
        StepReadiness(
    ...
    if fin_total > 0 and rsk_total == 0:
        s5_missing.append(
            MissingItem(
                type="action",
                label="Keine Risiken aus Findings abgeleitet",
                count=fin_total,
                href="/risks",
            )
        )
        ...
        s5_missing.append(
            MissingItem(
                type="action",
                label="Findings ohne Risikoverknüpfung",
                count=fin_no_risk,
                href="/risks",
            )
        )
    steps.append(
        StepReadiness(
            ...
            score=100
            if rsk_total > 0 and fin_no_risk == 0
            else (50 if rsk_total > 0 else (0 if fin_total > 0 else 100)),
    ...
    if rsk_total > 0 and rec_total == 0:
        s6_missing.append(
            MissingItem(
                type="action",
                label="Keine Empfehlungen zu offenen Risiken erstellt",
                count=rsk_total,
    ...
    if rec_overdue > 0:
        s7_missing.append(
            MissingItem(
                type="action",
                label="Überfällige Maßnahmen ohne Umsetzungsnachweis",
                count=rec_overdue,
                href="/recommendations",
            )
        )
    steps.append(
        StepReadiness(
            ...
            score=100
            if not s7_missing
            else max(0, 100 - int((rec_overdue / max(rec_open, 1)) * 100)),
            open_count=rec_open,
            missing=s7_missing,
        )
    )
    ...
    if rec_open > 0:
        s8_missing.append(
            MissingItem(
                type="action",
                label="Offene Maßnahmen warten auf Verifikation",
                count=rec_open,
                href="/recommendations",
            )
        )
    ...
    if rep_recent == 0:
        s9_missing.append(
            MissingItem(
                type="action",
                label="Kein Bericht in den letzten 90 Tagen erstellt",
                count=0,
                href="/reports",
            )
        )
