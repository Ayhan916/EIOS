...
async def revise_assessment(
    ...
    if assessment is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Assessment not found")
    _assert_org_access(assessment.organization_id, current_user.organization_id)
    if assessment.review_status == ReviewStatus.APPROVED:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Assessment is approved and immutable. Only transition to ARCHIVED is permitted (ADR-014).",
        )
...
async def submit_for_review(
    ...
    if findings:
        ...
        all_links = await link_repo.list_by_assessment_findings(finding_ids)
        linked_ids = {lnk.finding_id for lnk in all_links}
        unlinked = [fid for fid in finding_ids if fid not in linked_ids]
        if unlinked:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail={
                    "message": (
                        f"{len(unlinked)} finding(s) have no evidence links. "
                        "Add at least one evidence reference per finding before submitting for review (ADR-003)."
                    ),
                    "finding_ids_without_evidence": unlinked,
                },
            )
...
async def submit_review_action(
    ...
    except ImmutableEntityError as exc:
