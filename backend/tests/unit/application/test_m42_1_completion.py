...
class TestEnterpriseRollups:
    ...
    def test_emissions_rollup_empty_org_list(self):
        from application.sustainability.rollup_service import _emissions_rollup, EmissionsRollup
        session = _mock_session()
...
class TestBusinessUnitRollups:
    ...
    def test_objectives_rollup_empty_org_list(self):
        from application.sustainability.rollup_service import _objectives_rollup, ObjectivesRollup
...
class TestRegionRollups:
    ...
    def test_kpis_rollup_empty(self):
        from application.sustainability.rollup_service import _kpis_rollup, KPIsRollup
...
class TestLegalEntityRollups:
    ...
    def test_climate_rollup_returns_none_scores_when_empty(self):
        from application.sustainability.rollup_service import _climate_rollup, ClimateRiskRollup
        session = _mock_session()
...
class TestProgramValidation:
    ...
    def test_create_objective_calls_validate_when_program_id_given(self):
        ...
        with patch("application.sustainability.objective_service.emit_audit_event"):
            with patch("application.sustainability.objective_service.validate_program_assignment") as mock_vpa:
                objective_service.create_objective(
                    "org-1", "Green Energy", "ENVIRONMENTAL", "actor-1", session,
                    program_id="prog-1",
                )
    ...
    def test_create_objective_skips_validate_when_no_program_id(self):
        ...
        with patch("application.sustainability.objective_service.emit_audit_event"):
            with patch("application.sustainability.objective_service.validate_program_assignment") as mock_vpa:
                objective_service.create_objective(
                    "org-1", "Water Usage", "ENVIRONMENTAL", "actor-1", session,
                )
...
class TestTargetsPageApiContract:
    ...
    def test_list_all_targets_returns_tuples(self):
        ...
        session.query.return_value.filter.return_value.order_by.return_value.limit.return_value.offset.return_value.all.return_value = [t]
