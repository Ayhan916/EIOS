...
def _make_search(chunks: list[dict] | None = None) -> AsyncMock:
    ...
    fake_chunks = [FakeChunk(c["text"], c.get("title", "Source")) for c in (chunks or [])]
...
class TestCalibrationPipelineOutput:
    ...
    @pytest.mark.asyncio
    async def test_nace_and_right_preserved(self, pipeline: SectorRiskCalibrationPipeline) -> None:
    ...
    @pytest.mark.asyncio
    async def test_status_is_pending(self, pipeline: SectorRiskCalibrationPipeline) -> None:
        dto = await pipeline.calibrate("13", CSDDDRight.FORCED_LABOUR)
        assert dto.status == CalibrationStatus.PENDING

    @pytest.mark.asyncio
    async def test_sources_populated(self, pipeline: SectorRiskCalibrationPipeline) -> None:
        dto = await pipeline.calibrate("13", CSDDDRight.FORCED_LABOUR)
        assert len(dto.sources) > 0

    @pytest.mark.asyncio
    async def test_reasoning_populated(self, pipeline: SectorRiskCalibrationPipeline) -> None:
        dto = await pipeline.calibrate("13", CSDDDRight.FORCED_LABOUR)
        assert dto.reasoning
        assert len(dto.reasoning) > 5

    @pytest.mark.asyncio
    async def test_id_is_uuid_string(self, pipeline: SectorRiskCalibrationPipeline) -> None:
        import uuid
    ...
    @pytest.mark.asyncio
    async def test_created_at_is_set(self, pipeline: SectorRiskCalibrationPipeline) -> None:
        dto = await pipeline.calibrate("29", CSDDDRight.CHILD_LABOUR)
...
class TestProbabilityClamping:
    ...
    async def test_probability_above_10_clamped(self) -> None:
        pipeline = SectorRiskCalibrationPipeline(
            llm=_make_llm(
                {"probability": 15, "confidence": "High", "reasoning": "x", "key_sources": []}
            ),
            knowledge_search=_make_search(),
        )
        dto = await pipeline.calibrate("13", CSDDDRight.FORCED_LABOUR)
    ...
    async def test_probability_below_1_clamped(self) -> None:
        pipeline = SectorRiskCalibrationPipeline(
            llm=_make_llm(
                {"probability": -5, "confidence": "Low", "reasoning": "x", "key_sources": []}
            ),
            knowledge_search=_make_search(),
        )
...
class TestLLMFailureFallback:
    ...
    async def test_search_exception_still_returns_suggestion(self) -> None:
        ...
        pipeline = SectorRiskCalibrationPipeline(
            llm=_make_llm(
                {"probability": 5, "confidence": "Medium", "reasoning": "ok", "key_sources": []}
            ),
...
class TestResponseParsing:
    def test_parse_clean_json(self) -> None:
        pipeline = SectorRiskCalibrationPipeline(llm=MagicMock(), knowledge_search=AsyncMock())
...
class TestNoRAGFallback:
    ...
    async def test_sources_include_parametric_fallback_note(self) -> None:
        pipeline = SectorRiskCalibrationPipeline(
            llm=_make_llm(
                {"probability": 4, "confidence": "Low", "reasoning": "x", "key_sources": []}
            ),
            knowledge_search=_make_search([]),
        )
        dto = await pipeline.calibrate("29", CSDDDRight.CHILD_LABOUR)
