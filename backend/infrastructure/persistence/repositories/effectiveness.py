...
from domain.enums import EffectivenessReviewStatus as ReviewStatus, IndicatorDataSource, IndicatorType
...
SEED_INDICATORS = [
    ...
    ("Closed CAPs (12M)", "Number of corrective action plans closed in the last 12 months", "quantitative", "count", "automatic", "Art. 11", "corrective_actions"),
    ("Overdue CAPs", "Number of CAPs past their deadline", "quantitative", "count", "automatic", "Art. 11", "corrective_actions"),
    ("Grievance Response Time (Ø)", "Average days to first response on grievance cases", "quantitative", "days", "automatic", "Art. 14", "grievance"),
    ("Grievance Remediation Rate", "Percentage of grievances leading to a remedy case", "quantitative", "percent", "automatic", "Art. 12", "grievance"),
    ("Remedy Cases Closed (12M)", "Number of remedy cases closed in the last 12 months", "quantitative", "count", "automatic", "Art. 12", "remedy"),
    ("Stakeholder Consultations (12M)", "Number of formal stakeholder consultations conducted", "quantitative", "count", "automatic", "Art. 13", "stakeholder"),
    ("High-Risk Suppliers (%)", "Share of suppliers rated high-risk in current assessments", "quantitative", "percent", "automatic", "Art. 8", "risk"),
    ("Audit Coverage (%)", "Percentage of tier-1 suppliers assessed in the last 12 months", "quantitative", "percent", "automatic", "Art. 8", "audit"),
    ("Average Risk Score Δ (CAPs)", "Mean risk score reduction after CAP closure", "quantitative", "score", "automatic", "Art. 15", "risk"),
    # Quantitative — manual
    ("Policy Awareness Rate", "Percentage of employees aware of the DD policy", "quantitative", "percent", "manual", "Art. 7", "governance"),
    ("CoC Acceptance Rate (%)", "Percentage of suppliers who have accepted the Code of Conduct", "quantitative", "percent", "manual", "Art. 7", "governance"),
    ("Training Completion Rate (%)", "Percentage of relevant staff completing DD training", "quantitative", "percent", "manual", "Art. 7", "human_rights"),
    ("Incident Recurrence Rate (%)", "Percentage of closed findings that recurred within 24 months", "quantitative", "percent", "manual", "Art. 15", "risk"),
    # Qualitative — manual
    ("Stakeholder Satisfaction", "Overall satisfaction of stakeholders with DD engagement process", "qualitative", "1-5 scale", "manual", "Art. 13", "stakeholder"),
    ("Audit Finding Trend", "Directional assessment of findings trend vs. prior period", "qualitative", "improving/stable/worsening", "manual", "Art. 8", "audit"),
    ("Management Commitment", "Assessment of board-level commitment to DD implementation", "qualitative", "1-5 scale", "manual", "Art. 22", "governance"),
    ("Process Maturity", "Maturity level of DD processes across the organisation", "qualitative", "1-5 scale", "manual", "Art. 15", "governance"),
    ("Supplier Collaboration Quality", "Quality of supplier engagement in DD activities", "qualitative", "1-5 scale", "manual", "Art. 10", "supplier"),
    ("Grievance Mechanism Accessibility", "Effectiveness of the grievance mechanism for affected parties", "qualitative", "1-5 scale", "manual", "Art. 14", "grievance"),
    ("Policy Effectiveness Assessment", "Expert assessment of whether DD policy achieves stated objectives", "qualitative", "1-5 scale", "manual", "Art. 7", "governance"),
]
...
class SQLEffectivenessIndicatorRepository:
    ...
    def seed_if_empty(self) -> None:
        count = self.db.query(EffectivenessIndicatorModel).filter(
        ...
        for name, desc, ind_type, unit, source, article, risk_cat in SEED_INDICATORS:
            self.db.add(EffectivenessIndicatorModel(
                id=uuid4(),
    ...
    def list(self, org_id: UUID, risk_category: Optional[str] = None) -> list[EffectivenessIndicator]:
        q = self.db.query(EffectivenessIndicatorModel).filter(
            (EffectivenessIndicatorModel.organization_id.is_(None)) |
            (EffectivenessIndicatorModel.organization_id == org_id)
        ).filter(EffectivenessIndicatorModel.is_active == True)
        if risk_category:
    ...
    def get(self, indicator_id: UUID) -> Optional[EffectivenessIndicator]:
        m = self.db.query(EffectivenessIndicatorModel).filter(EffectivenessIndicatorModel.id == indicator_id).first()
...
class SQLEffectivenessReviewRepository:
    ...
    def get(self, review_id: UUID, org_id: UUID) -> Optional[EffectivenessReview]:
        m = (
            self.db.query(EffectivenessReviewModel)
            .filter(EffectivenessReviewModel.id == review_id, EffectivenessReviewModel.organization_id == org_id)
            .first()
        )
        if not m:
            return None
        lines = [_line_to_domain(l) for l in self.db.query(ReviewLineModel).filter(ReviewLineModel.review_id == review_id).all()]
        return _review_to_domain(m, lines)

    def list_by_org(self, org_id: UUID, skip: int = 0, limit: int = 50) -> list[EffectivenessReview]:
        rows = (
            self.db.query(EffectivenessReviewModel)
        ...
        result = []
        for m in rows:
            lines = [_line_to_domain(l) for l in self.db.query(ReviewLineModel).filter(ReviewLineModel.review_id == m.id).all()]
            result.append(_review_to_domain(m, lines))
        return result

    def update(self, review_id: UUID, org_id: UUID, data: dict) -> Optional[EffectivenessReview]:
        m = (
            self.db.query(EffectivenessReviewModel)
            .filter(EffectivenessReviewModel.id == review_id, EffectivenessReviewModel.organization_id == org_id)
            .first()
    ...
    def submit(self, review_id: UUID, org_id: UUID, submitted_by: str) -> Optional[EffectivenessReview]:
        m = (
            self.db.query(EffectivenessReviewModel)
            .filter(EffectivenessReviewModel.id == review_id, EffectivenessReviewModel.organization_id == org_id)
            .first()
    ...
    def close(self, review_id: UUID, org_id: UUID, approved_by: str) -> Optional[EffectivenessReview]:
    ...
    def upsert_line(self, review_id: UUID, data: dict) -> ReviewLine:
        existing = (
            ...
            .filter(ReviewLineModel.review_id == review_id, ReviewLineModel.indicator_id == data["indicator_id"])
            .first()
        )
        if existing:
...
class SQLCAPSnapshotRepository:
    ...
    def get_snapshot(self, cap_id: str, org_id: UUID) -> Optional[CAPEffectivenessSnapshot]:
        m = (
        ...
        return CAPEffectivenessSnapshot(
            ...
            closed_score=closed,
            risk_delta=round(closed - baseline, 2) if (baseline is not None and closed is not None) else None,
...
class SQLEffectivenessDashboardRepository:
    ...
    def get_dashboard(self, org_id: UUID) -> dict:
        ...
        snapped = [c for c in all_caps if getattr(c, "baseline_score", None) is not None and getattr(c, "closed_score", None) is not None]
        avg_risk_delta = (
        ...
        return {
            ...
            "escalation_needed": overdue_caps > 5 or (avg_risk_delta is not None and avg_risk_delta > 1.0),
