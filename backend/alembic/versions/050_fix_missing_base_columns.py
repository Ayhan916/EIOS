...
def _add(table: str, cols: list[str]) -> None:
    for col in cols:
        if col == "status":
            ...
            op.execute(f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS owner VARCHAR(36)")
        elif col == "created_by":
            op.execute(f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS created_by VARCHAR(36)")
        elif col == "updated_by":
            op.execute(f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS updated_by VARCHAR(36)")
