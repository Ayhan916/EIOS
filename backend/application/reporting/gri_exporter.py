...
def build_gri_report(
    *,
    ...
    if emissions:
        items.extend([
            GRIDisclosure("GRI 305", "305-1", "Direct (Scope 1) GHG emissions",
            ...
            GRIDisclosure("GRI 305", "305-2", "Energy indirect (Scope 2) GHG emissions",
                          str(emissions.get("scope2_market")) if emissions.get("scope2_market") is not None else None,
                          "tCO2e",
            ...
            GRIDisclosure("GRI 305", "305-3", "Other indirect (Scope 3) GHG emissions",
                          ...
                          None if emissions.get("scope3") is not None else "Not all Scope 3 categories measured"),
        ])

    # ── GRI 302: Energy ───────────────────────────────────────────────────────
    if emissions and emissions.get("energy_mwh") is not None:
        items.append(GRIDisclosure("GRI 302", "302-1", "Energy consumption within the organization",
                                   str(emissions["energy_mwh"]), "MWh",
    ...
    if workforce:
        items.extend([
            GRIDisclosure("GRI 401", "401-1", "New employee hires and employee turnover",
                          str(workforce.get("turnover_pct")) if workforce.get("turnover_pct") is not None else None,
                          "%",
                          "Annual employee turnover rate.",
                          "reported" if workforce.get("turnover_pct") is not None else "omitted"),
            GRIDisclosure("GRI 405", "405-2", "Ratio of basic salary and remuneration of women to men",
                          str(workforce.get("pay_gap_pct")) if workforce.get("pay_gap_pct") is not None else None,
                          "%",
...
def build_gri_csv(report: dict[str, Any]) -> str:
    ...
    writer.writerow([
        "standard_code", "disclosure_id", "title",
        "value", "unit", "status", "omission_reason", "narrative",
    ])
