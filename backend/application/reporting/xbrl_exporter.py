...
def build_ixbrl(
    *,
    ...
    ix_hidden = etree.SubElement(ix_header, f"{{{_NS['ix']}}}hidden")

    # contexts
    ix_refs = etree.SubElement(ix_header, f"{{{_NS['ix']}}}references")
    ...
    etree.SubElement(ctx_period, f"{{{_NS['xbrli']}}}startDate").text = reporting_period_start.isoformat()
    etree.SubElement(ctx_period, f"{{{_NS['xbrli']}}}endDate").text = reporting_period_end.isoformat()
    ...
    if esrs_e1:
        ...
        _xbrl_numeric(body, "esrs-e1:GrossScope1GHGEmissions",
                      esrs_e1.get("scope1_tco2e"), ctx_id, _UNIT_TONNE,
                      "Gross Scope 1 GHG Emissions (tCO₂e)")
        _xbrl_numeric(body, "esrs-e1:GrossScope2GHGEmissionsMarketBased",
                      esrs_e1.get("scope2_market_tco2e"), ctx_id, _UNIT_TONNE,
                      "Gross Scope 2 GHG Emissions — Market-based (tCO₂e)")
        _xbrl_numeric(body, "esrs-e1:GrossScope2GHGEmissionsLocationBased",
                      esrs_e1.get("scope2_location_tco2e"), ctx_id, _UNIT_TONNE,
                      "Gross Scope 2 GHG Emissions — Location-based (tCO₂e)")
        _xbrl_numeric(body, "esrs-e1:GrossScope3GHGEmissions",
                      esrs_e1.get("scope3_tco2e"), ctx_id, _UNIT_TONNE,
                      "Gross Scope 3 GHG Emissions (tCO₂e)")
        _xbrl_numeric(body, "esrs-e1:TotalEnergyConsumption",
                      esrs_e1.get("energy_mwh"), ctx_id, _UNIT_MWH,
                      "Total Energy Consumption (MWh)")
        _xbrl_numeric(body, "esrs-e1:EnergyFromRenewableSources",
                      esrs_e1.get("renewable_energy_mwh"), ctx_id, _UNIT_MWH,
                      "Energy from Renewable Sources (MWh)")

    if esrs_e2:
        _heading(body, "ESRS-E2: Pollution")
        _xbrl_numeric(body, "esrs-e2:EmissionsToAir",
                      esrs_e2.get("emissions_to_air_tonnes"), ctx_id, _UNIT_TONNE,
                      "Emissions to Air (tonnes)")
        _xbrl_numeric(body, "esrs-e2:EmissionsToWater",
                      esrs_e2.get("emissions_to_water_tonnes"), ctx_id, _UNIT_TONNE,
                      "Emissions to Water (tonnes)")
        _xbrl_numeric(body, "esrs-e2:EmissionsToSoil",
                      esrs_e2.get("emissions_to_soil_tonnes"), ctx_id, _UNIT_TONNE,
                      "Emissions to Soil (tonnes)")

    if esrs_s1:
        _heading(body, "ESRS-S1: Own Workforce")
        _xbrl_numeric(body, "esrs-s1:TotalNumberOfEmployees",
                      esrs_s1.get("employee_headcount"), ctx_id, _UNIT_PERCENT,
                      "Total Number of Employees")
        _xbrl_numeric(body, "esrs-s1:NumberOfNonEmployeeWorkers",
                      esrs_s1.get("contractor_count"), ctx_id, _UNIT_PERCENT,
                      "Number of Non-Employee Workers (Contractors)")
        _xbrl_numeric(body, "esrs-s1:AnnualEmployeeTurnoverRate",
                      esrs_s1.get("annual_turnover_rate_pct"), ctx_id, _UNIT_PERCENT,
                      "Annual Employee Turnover Rate (%)")
        _xbrl_numeric(body, "esrs-s1:GenderPayGap",
                      esrs_s1.get("gender_pay_gap_pct"), ctx_id, _UNIT_PERCENT,
                      "Gender Pay Gap (%)")
        _xbrl_numeric(body, "esrs-s1:AverageTrainingHoursPerEmployee",
                      esrs_s1.get("avg_training_hours"), ctx_id, _UNIT_PERCENT,
                      "Average Training Hours per Employee")

    return etree.tostring(html, pretty_print=True, xml_declaration=False,
...
def _xbrl_numeric(parent, name: str, value, ctx_ref: str, unit_ref: str, label: str):
    ...
    if value is None:
        span_nil = etree.SubElement(p, f"{{{_NS['ix']}}}nonFraction",
                                    name=name,
                                    contextRef=ctx_ref,
                                    unitRef=unit_ref,
                                    decimals="2",
        ...
        span_val = etree.SubElement(p, f"{{{_NS['ix']}}}nonFraction",
                                    name=name,
                                    contextRef=ctx_ref,
                                    unitRef=unit_ref,
                                    decimals="2")
