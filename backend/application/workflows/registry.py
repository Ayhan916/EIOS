...
_REGISTRY: dict[str, WorkflowDefinition] = {
    d.workflow_type: d for d in (DUE_DILIGENCE, QUICK_SCAN, EVIDENCE_ANALYSIS, GOVERNANCE_REVIEW)
}
...
def get_workflow_definition(workflow_type: str) -> WorkflowDefinition:
    ...
    if definition is None:
        raise ValueError(f"Unknown workflow type: '{workflow_type}'. Valid types: {WORKFLOW_TYPES}")
    return definition
