...
class NewsScenarioDetector:
    ...
    def detect(
        ...
        hits: dict[ScenarioType, dict[str, list[dict]]] = {st: {} for st in ScenarioType}
        ...
        for scenario_type, nace_buckets in hits.items():
            ...
            affected_nace = sorted(
                code for code, arts in nace_buckets.items() if code != "*" and len(arts) >= 2
            )

            # Collect sample headlines (top 3 by article count)
            all_articles = [a for arts in nace_buckets.values() for a in arts]
...
async def list_scenario_suggestions(
    ...
    stmt = (
        select(ScenarioSuggestionModel)
        .order_by(ScenarioSuggestionModel.created_at.desc())
        .limit(limit)
    )
