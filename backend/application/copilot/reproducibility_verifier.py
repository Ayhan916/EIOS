...
async def verify_audit_package(
    ...
    if pkg.organization_id != org_id:
        return VerificationResult(
            ...
            checks=[
                VerificationCheck(
                    "tenant_check", False, "Package does not belong to this organisation"
                )
            ],
        )
    ...
    checks.append(
        VerificationCheck(
            ...
            "Hash matches"
            if hash_ok
            else f"Hash mismatch: stored={pkg.package_hash[:16]}… recomputed={recomputed[:16]}…",
        )
    )
    ...
    if msg is None:
        checks.append(
            VerificationCheck("message_exists", False, "Original message not found in database")
        )
    else:
        ...
        checks.append(
            VerificationCheck(
                ...
                "Answer matches"
                if answer_ok
                else "Answer in package differs from stored message content",
            )
        )
        ...
        checks.append(
            VerificationCheck(
                "snapshot_integrity",
                snapshot_ok,
                "Retrieval snapshot keys match"
                if snapshot_ok
                else f"Snapshot key mismatch: package={sorted(pkg_retrievers)} message={sorted(msg_retrievers)}",
            )
        )
