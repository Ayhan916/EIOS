  ...
  return (
    <div className="min-h-screen bg-slate-50 p-8 print:p-4">
      ...
      {isLoading ? (
        ...
        <p className="text-center text-muted-foreground py-20">No executive data available.</p>
      ) : (
        <>
          {/* ── 5 pillars ────────────────────────────────────────────────── */}
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-5">
            ...
            <MetricPillar
              ...
              sub={cc.yoy.avg_esg_delta != null ? `${cc.yoy.avg_esg_delta >= 0 ? "+" : ""}${cc.yoy.avg_esg_delta.toFixed(1)} YoY` : "avg across portfolio"}
          ...
          <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Overall ESG Health</p>
              ...
              <div className="text-right space-y-1">
                <div className="flex items-center gap-2 justify-end text-sm">
                  ...
                  <span>{cc.ceo.total_scored_suppliers} scored suppliers</span>
                </div>
                <div className="flex items-center gap-2 justify-end text-sm">
                  <BarChart3 className="h-4 w-4 text-blue-500" />
                  <span>{cc.cfo.taxonomy_alignment_pct?.toFixed(1) ?? "—"}% taxonomy aligned</span>
                </div>
                <div className="flex items-center gap-2 justify-end text-sm">
                  <AlertTriangle className="h-4 w-4 text-red-500" />
                  <span>{cc.cco.open_critical_findings} critical findings open</span>
                </div>
              </div>
            </div>
          </div>
