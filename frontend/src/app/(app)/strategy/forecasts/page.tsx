...
const WIZARD_STEPS = [
  { title: "Create a Forecast Model", desc: "Start by defining a model — choose the methodology (Linear Trend, Weighted Average, or Scenario Projection) and give it a name." },
  { title: "Configure Your Target Metric", desc: "Select what to forecast: KPI performance, emissions trajectory, green revenue, or EU Taxonomy alignment." },
  { title: "Run a Forecast", desc: "Click 'Forecast starten' to generate deterministic, auditable projections based on your historical data." },
  { title: "Compare Scenarios", desc: "Once you have 2+ results, use the comparison view to evaluate different pathways side by side." },
];
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          {isLast ? (
            ...
            >
              Create first model <ArrowRight className="h-3 w-3" />
            </button>
          ) : (
            <button
...
const METHODOLOGIES = [
  { value: "LINEAR_TREND", label: "Linearer Trend" },
  { value: "WEIGHTED_MOVING_AVERAGE", label: "Gewichteter Durchschnitt" },
  { value: "SCENARIO_PROJECTION", label: "Szenario-Projektion" },
];

const FORECAST_TYPES = [
  { value: "KPI", label: "KPI" },
  { value: "EMISSIONS", label: "Emissionen" },
...
function CreateModelModal({ orgId, onClose }: { orgId: string; onClose: () => void }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    if (!form.model_name.trim()) { setError("Name erforderlich"); return; }
    mut.mutate({
      ...
      <div className="w-full max-w-md rounded-xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold">Neues Forecast-Modell</h2>
          </div>
...
function RunForecastModal({ orgId, onClose }: { orgId: string; onClose: () => void }) {
  ...
  function submit(e: React.FormEvent) {
    e.preventDefault(); setError(null);
    if (!form.forecast_model_id) { setError("Modell wählen"); return; }
    if (!form.target_metric.trim()) { setError("Zielmetrik angeben"); return; }
    const year = parseInt(form.forecast_year, 10);
    const baseline = parseFloat(form.baseline_value);
    if (isNaN(year) || isNaN(baseline)) { setError("Jahr und Basiswert müssen Zahlen sein"); return; }
    mut.mutate({
      ...
      <div className="w-full max-w-md rounded-xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold">Forecast ausführen</h2>
          </div>
          <button onClick={onClose} className="rounded-md p-1 hover:bg-slate-100">
            <X className="h-5 w-5 text-slate-500" />
          </button>
        ...
        <form onSubmit={submit} className="space-y-4 px-6 py-5">
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">Modell <span className="text-red-500">*</span></label>
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">Zielmetrik <span className="text-red-500">*</span></label>
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">Basiswert <span className="text-red-500">*</span></label>
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">Szenario (optional)</label>
...
function ForecastComparePanel({ results }: { results: ForecastResult[] }) {
  ...
  const fields: Array<{ label: string; key: keyof ForecastResult }> = [
    ...
    { label: "Forecast Year", key: "forecast_year" },
    { label: "Baseline Value", key: "baseline_value" },
    { label: "Forecast Value", key: "forecast_value" },
    { label: "Lower Bound", key: "lower_bound" },
    { label: "Upper Bound", key: "upper_bound" },
    { label: "Confidence Level", key: "confidence_level" },
  ];
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">Forecast A</label>
            <select
              value={idA}
          ...
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">Forecast B</label>
            <select
              value={idB}
            ...
            <table className="w-full text-sm border-separate border-spacing-0">
              <thead>
                <tr className="text-xs text-muted-foreground">
                  <th className="py-2 text-left pr-4 font-medium">Kennzahl</th>
                  <th className="py-2 text-right font-medium text-violet-700">Forecast A</th>
                  <th className="py-2 text-right font-medium text-blue-700">Forecast B</th>
        ...
        {(!a || !b) && (
          <p className="text-center text-sm text-muted-foreground py-4">
            Wähle zwei Forecasts aus, um sie zu vergleichen.
          </p>
        )}
      </CardContent>
    </Card>
...
export default function ForecastsPage() {
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => setModal("run")} className="flex items-center gap-2 bg-violet-600 hover:bg-violet-700">
            <Play className="h-4 w-4" />Forecast starten
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card><CardContent className="pt-6"><p className="text-sm text-muted-foreground">Modelle</p><p className="mt-1 text-3xl font-bold">{(models ?? []).length}</p></CardContent></Card>
        <Card><CardContent className="pt-6"><p className="text-sm text-muted-foreground">Prognosen</p><p className="mt-1 text-3xl font-bold">{(results ?? []).length}</p></CardContent></Card>
      </div>

      {chartData.length > 0 && (
        <Card>
          <CardHeader><CardTitle>Prognose-Verlauf</CardTitle></CardHeader>
          <CardContent>
      ...
      <Card>
        <CardHeader><CardTitle>Forecast-Modelle</CardTitle></CardHeader>
        <CardContent>
          {(models ?? []).length === 0 ? (
                ...
                <div key={m.id} className="flex items-center justify-between rounded-lg border px-4 py-3 hover:bg-slate-50">
                  <div>
                    ...
                    <span className="mt-1 inline-block rounded bg-slate-100 px-2 py-0.5 text-xs text-slate-600">
                      {METHODOLOGIES.find((x) => x.value === m.methodology)?.label ?? m.methodology}
                    </span>
                  </div>
                  <div className="text-right text-xs text-muted-foreground">
                    <p>v{m.model_version}</p>
                    {m.is_approved && <p className="font-medium text-green-600">Genehmigt</p>}
                  </div>
                </div>
              ))}
            </div>
      ...
      {(results ?? []).length > 0 && (
        <Card>
          <CardHeader><CardTitle>Forecast-Ergebnisse</CardTitle></CardHeader>
          <CardContent>
            <div className="space-y-2">
              {(results ?? []).map((r) => (
                <div key={r.id} className="flex items-center justify-between rounded-lg border px-4 py-3 hover:bg-slate-50">
                  <div>
                    ...
                    <div className="mt-1 flex gap-2">
                      <span className="rounded bg-blue-100 px-2 py-0.5 text-xs text-blue-700">
                        {FORECAST_TYPES.find((t) => t.value === r.forecast_type)?.label ?? r.forecast_type}
                      </span>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-muted-foreground">Basis: {r.baseline_value?.toLocaleString("de-DE") ?? "—"}</p>
                  </div>
                </div>
              ))}
            </div>
